...
%\item Fortalezas
%\item[\done] Se logro armar una herramienta capaz de generar scripts en base al modelo.
%\item[\done] Facilita la administración. 
%\item[\done] Facilita la entrada a estas herramientas y a la %configuración de redes. No se necesitan conocimientos de %infraestructura para utilizarla.
%\item Se provee de una interfaz gráfica para el manejo de redes.
%\item[\done] Se obtienen las ventajas de usar CMTs.
%\item[\done] La estandarización (extensión) del modelado puede %permitir la integración con otras herramientas.

%\begin{itemize}
    %\item Debilidades
    %\begin{todolist}
    %\item[\done] Flexibilidad actual para la generación de scripts.
    %\item[\done] Pocos elementos de software disponibles.
    %\item[\done] Es necesario definir cada componente a nivel de metamodelo.
    %\item[\done] Aún se necesita un mínimo de conocimiento de CMTs para utilizar los scripts generados.
    %\end{todolist}
%\end{itemize}

%\begin{itemize}
%\item Posibilidades de extensión de la herramienta
%  \begin{todolist}
%  \item[\done] Posibilidad de adaptar a diferentes Configuration Managers (Chef, Ansible).
%  \item[\done] Posibilidad de agregar más elementos de software/configuración (aparte de los ya incluidos: Apache, Tomcat, etc).
%  \item Posibilidad de agregar atributos a los elementos de software/configuración para permitir mayor libertad en la definición de los modelos.
%  \item[\done] Generar otros tipos de salida (Generación de reportes, o ejemplos que no se lleguen a implementar en el proyecto actual).
%  \item[\done] Generar mecanismos sencillos de extensión de las transformaciones.
%  \end{todolist}
%\end{itemize}

%Motivaciones Proyecto
%Proveer una interfaz más accesible a personas sin conocimiento técnico de redes, para que sean capaces de diseñar y manejar grupos de nodos y configuraciones de redes simples.
%Proveer de un componente gráfico para facilitar la administración de una infraestructura, facilitando los cambios al tener un mecanismo de generación automatizado de scripts.
%El público al que va dirigido es a personas que no deseen tener un conocimiento extenso de los Configuration Managers, al dar la posibilidad de generar la transformación a partir de elementos gráficos. No va dirigido a expertos en dichas herramientas o administradores de redes que quieran tener una configuración detallada y específica (fine tuning).
